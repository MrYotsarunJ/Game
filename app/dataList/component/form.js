...
export default function FormComponent(props) {
  ...
  const id = props.id;
  const title_name = props.id ? "Edit Data" : "Add Data";
  const carData = [
    {
  ...
  const dataSource = [...carData, ...superCarData, ...superBikeData];
    ...
    if (id) {
      ...
      if (data) {
        form.setFieldsValue({
          ...
          model: data.model,
          year: data.year,
          price: data.price.replace("$", ""), // Remove '$' for input field
          type: data.type, // Set type in the form
        });
      }
      setLoading(false);
    }
  ...
  const onFinish = (values) => {
    message.success(`${title_name} successful!`);
    router.push("/dataList"); // Redirect to home page after submission
  };

  const handleBack = () => {
    router.push("/dataList"); // Navigate back to the previous page
  };
    ...
    >
      ...
      >
        <Select>
          ...
          <Select.Option value="Super Car">Super Car</Select.Option>
          <Select.Option value="Super Bike">Super Bike</Select.Option>
        </Select>
      </Form.Item>
      ...
      >
        <InputNumber min={1900} max={2024} style={{ width: "100%" }} />
      </Form.Item>
      ...
      <Form.Item>
        <Button style={{ margin: 10, minWidth: 50 }} onClick={handleBack}>
          Back
        </Button>
